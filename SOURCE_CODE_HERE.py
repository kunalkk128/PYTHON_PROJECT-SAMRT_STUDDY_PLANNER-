...
def save_daily_log():
    ...
    if sub == "" or hrs == "" or date_val == "":
        messagebox.showwarning("Warning", "Saare fields bharna zaroori hai!")
        return

    try:
        hrs_val = float(hrs)
    except ValueError:
        messagebox.showerror("Error", "Hours me sahi number daalo!")
        return
    ...
    update_log_table()
    messagebox.showinfo("Saved", "Daily log CSV me save ho gaya!")
...
def show_analytics_chart():
    ...
    plt.bar(x - width/2, targets, width, label='Target Planned (Hrs)', color='#e74c3c')
    plt.bar(x + width/2, dones, width, label='Actual Completed (Hrs)', color='#2ecc71')

    plt.xlabel('Subjects')
    plt.ylabel('Hours')
    plt.title('Smart Study Planner: Target vs Actual Progress')
